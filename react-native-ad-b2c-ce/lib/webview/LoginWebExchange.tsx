...
export function LoginWebExchange({route, navigation}) {
    ...
    const exchangeCodes = async () => {
      ...
      try {
        ...
        if (result) {
          ...
          // since we do not know if Azure AD B2C is doing registration or just login, we
          // have to set the returned URL in the browser to see if it actually finalized the
          // flow or displays html
          setAuthUrl(result);
        } else {
          setError('Could not authorize. Please try again later...');
        }
