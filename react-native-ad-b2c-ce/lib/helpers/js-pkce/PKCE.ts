...
export default class PKCE {
  ...
  public authorizeUrl(additionalParams: IObject = {}): string {
    ...
    ).toString(); // MODIFICATION: added .toString
  ...
  // MODIFICATION: added pkceVerifier
  private pkceVerifier(): string {
    return Base64.stringify(WordArray.random(64))
      .replace(/\+/g, '-')
      .replace(/\//g, '_')
      .replace(/[=]+$/, '');
  }
  ...
  private randomStringFromStorage(key: string): string {
    ...
    if (fromStorage === null) {
      this.getStore().setItem(key, this.pkceVerifier()); // MODIFICATION: added pkceVerifier
    }
  ...
  // MODIFICATION: added reset
  public reset() {
    this.state = '';
    this.codeVerifier = '';

    this.getStore().setItem('pkce_code_verifier', undefined);
    this.getStore().setItem('pkce_state', undefined);
  }
}
